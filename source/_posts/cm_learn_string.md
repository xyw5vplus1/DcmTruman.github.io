...
- [x] [后缀自动机四](http://hihocoder.com/problemset/problem/1457) 给你个0~9组成的串，当成数组看。求本质不同的串的数字和（这道题让我想去以前网络赛一个“求本质不同回文子串的数字和”）。如果是单个串。每个节点记录当前节点所表示的串的和，转移的时候就是当前节点*10+转移边-'0'。如果是多个传，就用‘：’连接（因为‘：’是‘0’的ascii码+10）
